...
expName = 'social_predictability'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\nylah\\InfantAttentionStudy\\social_predictability.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # Cumulative counters — start at zero, accumulate across all trials
    cumulative_looking_time = 0
    cumulative_lookaway_time = 0
    ...
    for thisTrial in trials:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from attention_getter_code
        a_space_was_held_on_entry = space_down_state[0]
        ...
        thisExp.currentRoutine = attention_getter
        ...
        while continueRoutine:
            ...
            # Holding space advances to trial — hold carries into trial automatically
            if a_space_was_held_on_entry and not space_down_state[0]:
                a_space_was_held_on_entry = False
                
            if not a_space_was_held_on_entry and space_down_state[0]:
            ...
            if thisExp.status == PAUSED:
                pauseExperiment(
                    ...
                    win=win, 
                    timers=[routineTimer, globalClock], 
                    currentRoutine=attention_getter,
                )
                # skip the frame we paused on
                continue
            ...
            # has the Routine been forcibly ended?
            if attention_getter.forceEnded or routineForceEnded:
                break
            # has every Component finished?
            continueRoutine = False
            for thisComponent in attention_getter.components:
        ...
        print(f"Trial {trials.thisN +1} started")
        ...
        face_stim.size = (0.22, 0.22)
        face_stim.pos = [0, start_y]
        occluder_stim.size = (0.23, 0.23)
        occluder_stim.pos = [0, 0]
        
        # Looking time tracking — reset each trial
        total_looking_time = 0
        total_lookaway_time = 0
        
        # If space already held from attention getter, looking started at t=0
        if space_down_state[0]:
            look_start_time = 0
            lookaway_start_time = None
            print("Baby is Looking")
        else:
        ...
        while continueRoutine:
            ...
            # Detect look START (space pressed while tracking looking time)
            if space_is_down and look_start_time is None:
                look_start_time = t
                if lookaway_start_time is not None:
                    total_lookaway_time += t - lookaway_start_time
                    lookaway_start_time = None
                print(f"Looking started at t={t:.3f}s")
            
            # Detect look END (space released while tracking a lookaway)
            if not space_is_down and look_start_time is not None:
                total_looking_time += t - look_start_time
                look_start_time = None
                lookaway_start_time = t
                print(f"Looked for t={t:.3f}s")
            
            # 1 second of continuous lookaway → end trial
            if lookaway_start_time is not None and (t - lookaway_start_time) >= 1.0:
                print(f"Lookaway >= 1s — ending trial")
                continueRoutine = False
        ...
        # Finalize any in-progress look or lookaway episode
        ...
        # Per-trial data
        thisExp.addData('total_looking_time', round(total_looking_time, 3))
        thisExp.addData('total_lookaway_time', round(total_lookaway_time, 3))
        
        # Cumulative data (running total from first trial onward)
        cumulative_looking_time += total_looking_time
        cumulative_lookaway_time += total_lookaway_time
        thisExp.addData('cumulative_looking_time', round(cumulative_looking_time, 3))
        thisExp.addData('cumulative_lookaway_time', round(cumulative_lookaway_time, 3))
        
        # Stop current sound if still playing
        if current_sound is not None and current_sound.isPlaying:
            current_sound.stop()
        
        print(f"Trial {trials.thisN +1} ended")
