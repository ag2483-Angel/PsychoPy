...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrial in trials:
        ...
        # Use PsychoPy's cross-platform keyboard handler instead of pyglet's
        # raw keyboard state, which requires Accessibility permissions on macOS
        ...
        while continueRoutine:
            ...
            # Check for space press and release using PsychoPy's cross-platform keyboard
            space_keys = kb.getKeys(keyList=['space'], waitRelease=False, clear=True)
            for k in space_keys:
                if k.duration is None and not space_held:
                    # Key was just pressed down
                    space_held = True
                    space_start_time = t
                    print(f"Space pressed at {t}")
                elif k.duration is not None and space_held:
                    # Key was released
                    looking_time = k.duration
